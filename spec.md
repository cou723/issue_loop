...
各イテレーションは終了時に `.issue-loop/iteration-signal` へ `DONE` / `NO_ISSUE` / `CANCELLED` / `NEEDS_INPUT` / `FAILED` のいずれかを書き出す。オーケストレーターはイテレーション起動前に同ファイルを削除し、起動後に内容を読む。

待機はハーネスの完了通知を主とし、フォールバックの ScheduleWakeup を予約する場合も prompt にコマンド自体を渡さない（ユーザーの新規依頼と区別できず、完了後の再発火が依頼されていないループを開始する事故につながるため）。ループ終了時には残存 wakeup を必ず停止する。iteration エージェント内部のサブエージェント起動は同期実行とし、ビジーウェイト（sleep ポーリング）を行わない。シグナルが空（コンテキスト圧縮等でシグナル書き込みが漏れた場合）には即停止せず、PR の存在を確認して DONE 相当と判断できればループを継続する。PR も確認できない場合のみ異常終了としてループを停止する。
...
スコープ内に CRITICAL / HIGH / MEDIUM の指摘が1件以上あれば `fail`。指摘が0件、または LOW のみであれば `pass`（LOW を理由に修正ループを回さない。指摘自体は結果に残す）。

再レビュー（2回目以降）ではオーケストレーター（iteration）が前回のスコープ内指摘を prompt で引き継ぎ、review は対応確認を主目的とする。新規指摘をスコープ内へ加えられるのは「前回修正が導入した問題」か「CRITICAL / HIGH」のみで、軽微な指摘の後出しはスコープ外へ回す。これにより毎回新しい軽微な指摘でレビューが収束しない事態を防ぐ。
